...
DB_PATH="/usr/local/kervoaz/filebrowser.db"
DEFAULT_PORT=8080
...
ExecStartPre=/bin/touch /usr/local/kervoaz/filebrowser.db
ExecStartPre=/usr/local/bin/filebrowser config set -a "0.0.0.0" -p ${PORT} -d /usr/local/kervoaz/filebrowser.db
ExecStart=/usr/local/bin/filebrowser -r / -d /usr/local/kervoaz/filebrowser.db -p ${PORT}
Restart=always
